...
assert.match(iconSource, /function restoreFirstPaintIconRender\(icon, site, render\) \{[\s\S]*if \(render\.generic\) \{[\s\S]*applySiteIcon\(icon, site\);[\s\S]*return;/, "Generic fallback cache must not replay over Chrome's native favicon route.");
