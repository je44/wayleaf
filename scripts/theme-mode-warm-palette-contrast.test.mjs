...
assert.match(
  ...
  "Amber and coral dark palettes should use a stronger mode-control base and accent-tinted selected state."
);
...
assert.match(
  ...
  "Warm dark selected states should use depth and readable inactive labels without adding an outline."
);

assert.doesNotMatch(
  warmDarkPaletteBlock,
  /--theme-mode-active-shadow:[\s\S]*inset/,
  "Amber and coral dark mode controls should not add an inset outline that other palettes do not use."
);
