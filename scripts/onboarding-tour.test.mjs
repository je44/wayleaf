...
assert.doesNotMatch(styles, /\.onboarding-card\s*\{[^}]*(?:left|top) 260ms/, "The onboarding bubble must not animate layout properties.");
assert.match(source, /function positionOnboardingStep\(\) \{[\s\S]*Flip\.getState\(onboardingCard\)[\s\S]*onboardingCard\.style\.left = `\$\{left\}px`;[\s\S]*onboardingCard\.style\.top = `\$\{top\}px`;[\s\S]*Flip\.from\(previousState,[\s\S]*duration:\s*gsapDuration\(260\)/, "Onboarding steps should animate position changes through the existing GSAP Flip transform path.");
