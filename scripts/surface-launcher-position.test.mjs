...
assert.match(source, /refreshBookmarkFolderButton\.addEventListener\("click", \(\) => \{\s*clearBookmarkFolderViewCache\(\);\s*void renderSelectedBookmarkFolder\(\);\s*\}\);/, "Manual bookmark refresh must discard the rendered folder cache before reading current bookmark data.");
