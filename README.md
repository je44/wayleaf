...
<p align="center">
  ...
  搜索、快捷入口、书签、最近浏览、资讯和 AI 指令都放在一页。
</p>
...
Wayleaf 是一个 Chrome 新标签页扩展。它把常用入口、搜索、书签、最近浏览、资讯和 AI 指令放在同一页，让新建标签页变成开始下一步的地方。

它不需要账号，也没有后端服务。主题、入口、书签选择、置顶历史、搜索设置和同步状态都保存在 Chrome 扩展存储里。
...
| 看技术资讯 | 使用内置中英文资讯源，也可以添加 RSS/JSON 信息源。 |
...
- 备份迁移：导入、导出当前设置。
- 资讯来源：保留内置来源，也可以添加自己的 RSS/JSON 来源。
