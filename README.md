...
| --- | --- |
| 搜索或打开网页 | 搜索框支持本地历史/书签、完整 URL、Google、百度、Bing、Google+Bing 聚合搜索，以及 `*yt`、`*xhs` 等完整平台激活词。 |
...
| 输入完整网址后按回车 | 直接打开该网址。 |
| 输入完整的 `*平台缩写或名称`，例如 `*yt` 或 `*youtube` | 完整匹配后切换到对应平台；随后输入内容并回车。若短激活词也是其他平台的开头（如 `*x` / `*xhs`），按空格或回车确认 `*x`，继续输入则可完整匹配 `*xhs`。 |
