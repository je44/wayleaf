...
| --- | --- |
| 搜索或打开网页 | 搜索框支持本地历史/书签、完整 URL、Google、百度、Bing、Google+Bing 聚合搜索，以及 `*yt`、`*xhs` 等即时平台激活词。 |
...
| 输入完整网址后按回车 | 直接打开该网址。 |
| 输入完整的 `*平台缩写或名称`，例如 `*yt` 或 `*youtube` | 无需空格或搜索内容即可立即切换到对应平台；随后输入内容并回车。输入首字母或第二个字母时，搜索框右侧会提示可补全的激活词。 |
